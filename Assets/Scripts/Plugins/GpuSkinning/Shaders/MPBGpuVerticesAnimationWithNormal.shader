...
// 支持法线纹理的顶点动画 Shader (GPU Instancing + MaterialPropertyBlock)
Shader "GPUSkin/MPBGpuVerticesAnimationWithNormal" 
{
	Properties
	{
    ...
    HLSLINCLUDE
            ...
            Varyings Vertex(Attributes input)
			{
                ...
                // 从 MaterialPropertyBlock 获取动画数据
                ...
                float4 vertexUV = float4((vertexIndex) * _AnimationTex_TexelSize.x, 
                                         (frameIndex + 0.5) * _AnimationTex_TexelSize.y, 0, 0);
                ...
                if (blendProgress > 0.001)
                {
                    float4 blendVertexUV = float4(vertexIndex * _AnimationTex_TexelSize.x, 
                                                  (blendFrameIndex + 0.5) * _AnimationTex_TexelSize.y, 0, 0);
