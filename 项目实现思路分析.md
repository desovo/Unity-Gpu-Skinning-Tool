...
骨骼动画 Shader 中**没有对法线进行变换计算**。理论上，法线也应该通过骨骼矩阵进行变换：

```hlsl
// 理论上应该计算的法线变换（当前实现中缺失）
float3 normal =
...
但当前实现中：
- ❌ 法线保持静态，不随骨骼动画变换
- ⚠️ 对于需要正确光照的场景，这会导致明显的视觉问题
- 💡 如果需要正确的法线，建议使用顶点动画方式
...
虽然在数据生成阶段会创建法线动画纹理（`_AnimationNormalTex`），但在实际的 Shader 实现中：

1. **GpuVerticesAnimation.shader**：声明了 `_AnimationNormalTex` 但**完全未使用**。法线数据在渲染过程中被忽略，使用的是 Mesh 的静态法线。

2. **MPBGpuVerticesAnimation.shader**：法线纹理**仅在阴影投射通道**（ShadowCaster Pass）中使用：
   ```hlsl
   // 仅在 ShadowPassVertex 中使用
   float3 normal = tex2Dlod(_AnimationNormalTex, vertexUV1);
   output.positionCS = GetShadowPositionHClip(pos.xyz, normal);
   ```
   在主渲染通道中同样未使用动画法线。

3. **骨骼动画 Shader**：完全不计算或使用法线变换，顶点法线保持静态不变。

这意味着：
- ✅ 顶点位置会根据动画正确变换
- ❌ 法线方向不会随动画更新（除了 MPB 版本的阴影）
- ⚠️ 可能导致光照计算不准确，特别是在有明显形变的动画中

**改进建议**：
要完整支持法线动画，需要在 Vertex Shader 中：
1. 采样法线纹理获取动画法线
2. 将法线传递到 Fragment Shader
3. 在 Fragment Shader 中使用动画法线进行光照计算
...
3. **法线动画限制**（重要）：
   - **原始 Shader**：
     - **骨骼动画**：完全不计算法线变换，法线保持静态
     - **顶点动画**：虽然生成法线纹理，但大部分 Shader 中未使用（仅 MPB 版本的阴影通道使用）
     - ⚠️ 这会导致光照计算不准确，特别是在有明显形变的动画中
   - **✨ 新增 WithNormal Shader**：
     - ✅ `GpuSkinningAnimationWithNormal.shader` - 支持骨骼动画法线变换
     - ✅ `GpuVerticesAnimationWithNormal.shader` - 支持顶点动画法线纹理
     - ✅ 包含完整的光照系统（Lambert 漫反射 + 环境光）
     - ✅ 支持 URP 主光源和阴影
     - 📖 详见 `法线支持使用说明.md`
4. **网格要求**：
   - 所有子 Mesh 必须使用同一张主纹理
   - UV1 和 UV2 被用于骨骼数据，不能再用于其他用途
5. **不保留层级**：生成的 Prefab 是扁平化的单个 Mesh
6. **骨骼映射**：需要正确处理不同 SkinnedMeshRenderer 的骨骼索引映射
